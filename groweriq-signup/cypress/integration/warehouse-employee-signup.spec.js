context('Grower Signup Test', () => {
    ...
    it('Submit form by selecting only user type to "Warehouse Employee" and verifying error messages', () => {
        cy.get('#userType').get('#ddUserType').click()
        cy.get('.mat-select-panel').contains('Warehouse Employee').click({ force: true })
        cy.get('#userSignUpForm').submit()
        ...
        cy.get('#txtEduQual')
            .get('mat-error').should('be.visible').should('have.attr', 'role', 'alert').contains('Educational Qualification is a required field')
            .get('label').should('have.class', 'mat-form-field-empty').contains('Educational Qualification *')
    })
    ...
    it('Verify display "Warehouse Employee" specific controls', () => {
        cy.get('.mat-dialog-container')
        ...
        cy.get('.mat-dialog-container')
            ...
            .should('contain', 'Educational Qualification *')
            .should('contain', 'Are you Inventory Management Certified?')
        cy.get('.mat-dialog-container').find('#txtBatchesHandled').should('not.exist');
        cy.get('.mat-dialog-container').find('#txtYieldAcquired').should('not.exist');
        cy.get('.mat-dialog-container').find('#ddGreenhouseLocations').should('not.exist');
        cy.get('.mat-dialog-container').find('#txtYearsOfExp').should('exist');
        cy.get('.mat-dialog-container').find('#txtEduQual').should('exist');
        cy.get('.mat-dialog-container').find('#rdoInvMgmtCert').should('exist');
    ...
    it('Submit valid "Warehouse Employee" form with all field values and verify user detail & list page', () => {
        cy.get('#txtName').type('Warehouse Employee User')
        cy.get('#txtPhone').type('+1 9876543210')
        cy.get('#rdoGender').find('mat-radio-button:last').contains('Male').click({ force: true })
        cy.get('#txtStAddress').type('987 Xyz Drive')
        cy.get('#txtCity').type('Toronto')
        cy.get('#txtProvince').type('Ontario')
        cy.get('#txtCountry').type('Canada')
        cy.get('#txtPostalCode').type('M1Z 1R1')
        cy.get('#userType').get('#ddUserType').click()
        cy.get('.mat-select-panel').contains('Warehouse Employee').click({ force: true })
        cy.get('#txtYearsOfExp').type('5')
        cy.get('#txtEduQual').type('Masters')
        cy.get('#rdoInvMgmtCert').find('mat-radio-button:first').contains('Yes').click({ force: true })
        ...
        cy.get('.mat-dialog-content').get('.row')
            .first().should('contain', 'Name:').should('contain', 'Warehouse Employee User')
            .next().should('contain', 'Phone Number:').should('contain', '+1 9876543210')
            .next().should('contain', 'Gender:').should('contain', 'Male')
            .next().should('contain', 'User Type:').should('contain', 'Warehouse Employee')
            .next().should('contain', 'Address:').should('contain', '987 Xyz Drive, Toronto, Ontario, Canada, M1Z 1R1')
            .next().should('contain', 'Years of Experience:').should('contain', '5')
            .next().should('contain', 'Educational Qualification:').should('contain', 'Masters')
            .next().should('contain', 'Inventory Management Certification:').should('contain', 'Yes')
        cy.server()
        ...
        cy.get('mat-table').get('mat-row').first()
            .should('contain', 'Warehouse Employee User')
            .should('contain', '+1 9876543210')
            .should('contain', 'Warehouse Employee')
    })

    it('Submit valid "Warehouse Employee" form with only mandatory fields and verify user detail & list page', () => {
        cy.get('#txtName').type('Warehouse Employee User')
        cy.get('#txtPhone').type('+1 9876543210')
        cy.get('#userType').get('#ddUserType').click()
        cy.get('.mat-select-panel').contains('Warehouse Employee').click({ force: true })
        ...
        cy.get('.mat-dialog-content').get('.row')
            ...
            .next().should('contain', 'Years of Experience:').should('contain', '')
            .next().should('contain', 'Educational Qualification:').should('contain', 'Masters')
            .next().should('contain', 'Inventory Management Certification:').should('contain', 'N/A')
        cy.server()
        cy.route('GET', 'users').as('getUsersAPI')
        cy.get('.mat-raised-button').contains('Back to Registered Users List').click({ force: true })
        cy.wait('@getUsersAPI').its('status').should('eq', 200)
